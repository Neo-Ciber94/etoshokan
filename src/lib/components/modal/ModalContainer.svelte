...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
  <Dialog.Portal>
    ...
    {#each modals as modal, i (modal.id)}
      ...
      <div
        class="bg-background fixed top-[50%] left-[50%] z-50 w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] rounded-lg border p-6 shadow-lg animate-in fade-in-0 zoom-in-95 duration-200 sm:max-w-md"
        style="z-index: {51 + i}; transform: translate(-50%, -50%) translateY({(i - modals.length + 1) * 12}px) scale({1 - (modals.length - 1 - i) * 0.04});"
        role="dialog"
        aria-modal={isTop}
